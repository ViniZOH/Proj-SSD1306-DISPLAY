...
void ssd1306_draw_char(ssd1306_t *ssd, char c, uint8_t x, uint8_t y) {
  ...
  // Cálculo do índice na tabela de fontes
  if (c >= 'A' && c <= 'Z') {
      // Letras maiúsculas ('A' a 'Z')
      index = (c - 'A' + 11) * 8; // Maiúsculas começam no índice 11
  } else if (c >= '0' && c <= '9') {
      // Números ('0' a '9')
      index = (c - '0' + 1) * 8; // Dígitos começam no índice 1
  } else if (c >= 'a' && c <= 'z') {
      // Letras minúsculas ('a' a 'z')
      index = (c - 'a' + 37) * 8; // Minúsculas começam no índice 37
  } else {
      // Caractere não suportado, retorna sem desenhar
      return;
  }

  // Desenho do caractere no display
  for (uint8_t i = 0; i < 8; ++i) {
      uint8_t line = font[index + i]; // Lê a linha do caractere
